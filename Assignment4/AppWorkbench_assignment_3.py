...
methods = [estimate_integral_abs_error_MC, estimate_integral_abs_error_BMC]
num_estimates = [100, 10]
...
for k, ns in enumerate(ns_vector):
    ...
    for (i, method_label) in enumerate(methods_label):
        ...
        abs_error_estimates = method_function(ns, num_estimates[i], uniform_pdf)
...
plt.savefig(os.path.join('./out', 'lab_3_workbench_comparison.jpg'))
plt.close()
